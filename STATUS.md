...
**2026-09-23 (build 64):** Portrait sacrifice and target trays no longer clip the first and last cards. The sheet can shrink inside the viewport, and a portrait hand uses smaller cards with inset scroll padding so names stay readable. Music fades clamp volume to [0, 1], so a frame clock skew cannot set a negative volume or throw IndexSizeError. Club tour and patron confirm stay on build 63. `?v=64`.

**2026-09-23 (build 63):** The guided tour can no longer trap the player. A full-height target (the patron rail in portrait) used to set the tip’s `bottom` to about one screen tall, which parked Next and Skip above the viewport while the gold hole stayed up. The tip now clamps into the portrait and landscape viewport, and a too-tall target docks the sheet at the bottom while the hole still frames the target. Skip Tour is on every step, including step 0 of a tutorial match. Leave, a win, leaving the table, and Escape call `endTour` and hide the overlay. Advancing, skipping, or ending the tour clears a leftover hold-inspect dossier, and the tip sits above that dossier so it cannot cover Next or Skip. A patron on the live table (yours or the rival’s, including a locked deck such as Druid King) shows its real name and official text. Collection lock still hides patrons that are not in this match. If the patron cannot be called, Continue is hidden and Close dismisses the confirm (Cancel, tap-outside, and Escape do the same). `localSeat` on that path is already null-safe. Board CSS untouched. `?v=63`.
...
**2026-09-22 (build 58):** Board/felt finish-line on main after Club 56 and Friend PeerJS 57. Player-facing splash and cache-bust are **build 58**. Landscape top-seat hand and agents are full cards. A short tap plays, buys, calls, or picks. A one-second hold opens the dossier on tavern, hand, agents, patrons, and targeting trays and never also acts. Dismissing the dossier removes it from hit testing. Landscape patron inspect keeps every favor row on screen. Portrait packing from 51 stays. `?v=58`.
...
- [x] Login greet stamps the calendar; max 2 seasonal crates per month; loot follows the shown crate (Storm Atronach / Scalecaller / Flame Atronach / Dark Brotherhood; save ids iron / orichalcum / ebony / voidsteel)
